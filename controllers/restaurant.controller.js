...
const getListOfRestuarents = (req, res) => {
    ...
    restuarentModel.find()
      ...
      .catch((err) => {
        return res.status(400).json({ error: err, msg: "failed to restuarent data" });
      });
  };
...
const registerRestuarent = async (req , res)=>{
  const {restaurantName ,firstName,lastName , email , password, mobileNumber} = req.body

  if (!restaurantName || !firstName || !lastName  || !email || !password || !mobileNumber) {
...
const loginRestuarent = async (req, res) => {
  ...
  res.cookie("token",token,{expire: new Date()+9999})
    ...
    await restuarentModel.updateOne({ _id: restaurant._id }, { $set: { token: token } });
...
const generateToken = (id) => {
  ...
  module.exports = {getListOfRestuarents , registerRestuarent , getRestuarentById , loginRestuarent}